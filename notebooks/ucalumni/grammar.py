...
#
NAME_VIDE = (PNAME("nome1") + VIDE + PNAME("nome2")).setResultsName("name_with_vide")
NAME_WITH_PARENTESIS = (
    PNAME("nome")
    + pp.Optional(",")
    + LPARENT
    + pp.SkipTo(RPARENT).setResultsName("nota")
    + RPARENT
)
NAME_WITH_VIDE_PARENTESIS = (
    PNAME("nome1")
    + LPARENT
    + pp.SkipTo(RPARENT).setResultsName("nota")
    + RPARENT
...
NAME_ANY = (
    NAME_WITH_VIDE_PARENTESIS("remissão*")
    | NAME_VIDE("remissão")
    | NAME_WITH_PARENTESIS("anotação")
    | NAME_WITH_PARENTESIS_OPEN("anotação")
    | NAME_SIMPLE("normal")
)
...
FIELD_1 = pp.ZeroOrMore(pp.White()) + FIELD_NAME("fname") + ":" + FIELD_VALUE("fvalue")
FIELD_2 = pp.ZeroOrMore(pp.White()) + FIELD_NAME("fname") + "-" + FIELD_VALUE("fvalue")
...
FIELD_IDEM = (
    ...
    + FIELD_NAME("fname")
    + ":"
    + pp.SkipTo(", idem:").setResultsName("fvalue")
    + ", idem:"
    + FIELD_VALUE("idem")
)
...
# Special sequences:
# Provou ..... para ....
PROVOU = pp.Literal("Provou").setResultsName("fname") + pp.SkipTo(
    pp.StringEnd()
).setResultsName("fvalue")
PROVAS_DE = (
    pp.Literal("Provas").setResultsName("fname")
    + pp.Literal("de")
...
# Grau de .....
GRAU = (
    pp.Literal("Grau").setResultsName("fname")
    + pp.Literal("de")
    + pp.SkipTo(pp.StringEnd()).setResultsName("fvalue")
)
...
FILIACAO_PAI_MAE = (
    "Filiação:"
    + pp.Group(pp.SkipTo(" e de ")).setResultsName("pai")
    + "e de"
    + PNAME("mae")
)
...
class DateUtility:
    ...
    def __init__(self, d: Type[Union[str, pp.ParseResults]], start=0, end=-1):
        ...
        if d is None or d == "":
            ...
            DATELINE.setParseAction(preserve_original)  # this adds 'original' key
