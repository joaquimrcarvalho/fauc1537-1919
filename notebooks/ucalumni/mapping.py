...
def map_aluno_kperson(aluno: Aluno) -> n:
    ...
    """
    p = n(aluno.nome, aluno.sexo, id=aluno.id, obs=aluno.obs)
    # fill in the metadata from the catalog record
    p.include(atr("código-de-referência", aluno.codref, data=aluno.get_record_date()))
    p.include(
        atr("data-do-registo", aluno.get_record_date(), data=aluno.get_record_date())
    )
    ...
    p.include(
        ls("uc-saida", aluno.unit_date_final.value, data=aluno.unit_date_final.value)
    )
        ...
        if aluno.vide_target is not None:
            p.include(
                ls(
                    ...
                    aluno.vide_target,
                    aluno.unit_date_inicial.value,
                    obs=f"{aluno.nome}, vide {aluno.vide}",
                )
            )
...
def map_faculdade(aluno: Aluno, p: n):
    ...
    if problem is not None and problem > "":
        ...
        if aluno.faculdade_strange is not None and aluno.faculdade_strange > "":
            lsfstr = ls(
                "nota-processamento",
                "Faculdade corrigida, combinação incomum",
                today,
                obs=aluno.faculdade_strange,
            )
            p.include(lsfstr)
...
def map_matriculas(aluno: Aluno, p: n):
    for matricula in aluno.matriculas:
        ...
        lsm = ls(
            the_type, matricula.ambito.strip(), matricula.data.value, obs=matricula.obs
        )
        p.include(lsm)
        ...
        if matricula.modalidade > "":  # obrigado, ordinário, voluntário
            lsm3 = ls(
                f"{the_type}.{matricula.modalidade}",
                f"{matricula.ambito.strip()}",
                matricula.data.value,
                obs=matricula.obs,
            )
            p.include(lsm3)
            lsm4 = ls(
                f"{the_type}.{matricula.modalidade}" + ".ano",
                matricula.ambito.strip() + "." + ano,
                matricula.data.value,
                obs=matricula.obs,
            )
...
def map_instituta(aluno: Aluno, p: n):
    for instituta in aluno.instituta:
        lsm = ls(
            "instituta", instituta.data.value, instituta.data.value, obs=instituta.obs
        )
        p.include(lsm)
        ...
        lsm2 = ls("instituta.ano", ano, instituta.data.value, obs=instituta.obs)
        p.include(lsm2)
