...
def row_to_ksource(row, atrs=False):
    ...
    gt = fonte( fonte_id, tipo='UC-AUC-ficheiro-alunos',
               ...
               ref=reference,
               obs=obs)
    exclude_from_atrs = ['\ufeffID', 'RepositoryCode', 'UnitTitle',
                         "UnitDateInitial", "UnitDateFinal",
                         "DescriptionLevel",
                         "ScopeContent", "Arrangement",
                         'Creator', 'Created', 'Username', 'Processinfo', ]
    if atrs:
...
def row_to_klista(row, reference_date='2020-02-11', atrs=False):
    ...
    l = lista(gid, dia=day, tipo='lista-de-alunos', mes=month, ano=year,data=reference_date, loc=unit_id)
    exclude_from_atrs = ['RepositoryCode', 'UnitTitle', "UnitDateInitial",
                         "UnitDateFinal",
                         'Creator', 'Created', 'Username', 'Processinfo',
                         ]
    if atrs:
...
def row_to_n(row, atrs=False, direct_import=False) -> n:
    ...
    if pdate is not None: 
        record_date = datetime.strptime(pdate,
                                  "%d/%m/%Y %H:%M:%S")
    ...
    if atrs:
        exclude_from_atrs = ['\ufeffID', 'UnitTitle', "UnitDateInitial",
                             "UnitDateFinal", 'BiogHist', 'ScopeContent',
                             'Creator', 'Created', 'Username', 'Processinfo', ]
        for a, v in row.items():
            if v > ' ' and a not in exclude_from_atrs:
...
def export_alumni_source(sources_dir: str, kleio_group: kleio,
        testing=False,direct_import=False):
...
def import_auc_alumni(csv_file: str, dest_dir: str = '',        db_connection: str = None, 
    ...
    with open(csv_file) as csvfile:
        ...
        for row in auc_ficheiro:
            ...
            if description_level == 'SSR':
                ...
                kleio_file = kleio(
                    'gacto2.str', obs=f'Generated from Archeevo export file:  {csv_file} dated {file_reference_date}')
                ...
                ncount = ncount + 1
                if ncount > batch:
                    print('-- Number of records per batch reached, new batch generated.',
                          batch, f.id, aluno.nome)
                    export_alumni_source(
                            sources_dir, 
                            kleio_file, 
                            testing=testing,
